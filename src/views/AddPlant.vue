...
export default {
  ...
  methods: {
    ...
    pickMode(next) {
      ...
      if (next === "manual") this.manualSpecies = "";
    },
    resetMode() {
      this.photoData = null;
      this.manualSpecies = "";
    ...
    // ----- Camera flow: default species to Orchid -----
    onFile(e) {
      const file = e.target.files?.[0];
      if (!file) return;
      const reader = new FileReader();
      reader.onload = () => { this.photoData = reader.result; };
    ...
    savePhoto() {
      ...
      // All captured-by-camera plants default to Orchid
      this.$store.commit('setTempPlant', {
        source: 'camera',
        species: 'Orchid',
        name: 'Orchid',     // editable in Setup
        imageIndex: 0
        // photo: this.photoData  // keep if you want later
      });
